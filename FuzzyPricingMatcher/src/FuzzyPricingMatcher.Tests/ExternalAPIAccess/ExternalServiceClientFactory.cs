...
public sealed class ExternalServiceClientFactory : IExternalServiceClientFactory, IDisposable
{
    ...
    {
        if (!endpoint.Enabled)
            throw new ExternalAPIConfigurationException($"Endpoint '{endpointName}' is disabled.");
        if (!Uri.TryCreate(endpoint.BaseUrl, UriKind.Absolute, out var uri) || uri.Host.EndsWith(".invalid", StringComparison.OrdinalIgnoreCase))
            throw new ExternalAPIConfigurationException($"Endpoint '{endpointName}' does not have a permitted live host.");
        if (string.IsNullOrWhiteSpace(endpoint.Resource))
            throw new ExternalAPIConfigurationException($"Endpoint '{endpointName}' has no resource.");
        if (string.IsNullOrWhiteSpace(endpoint.Username) || string.IsNullOrWhiteSpace(endpoint.Password))
            throw new ExternalAPIConfigurationException($"Endpoint '{endpointName}' requires username and password.");
    }
}