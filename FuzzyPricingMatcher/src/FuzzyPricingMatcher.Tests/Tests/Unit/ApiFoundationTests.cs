...
public sealed class ApiFoundationTests
{
    ...
    {
        using var factory = new RestClientFactory();
    ...
    public void Resource_builder_supports_path_query_and_endpoint_specific_formats()
    {
        ...
        var path = builder.Build(Endpoint("https://api.example.test", "Path", "yyyyMMdd"), new DateOnly(2026, 9, 18));
        var appendedPath = builder.Build(Endpoint("https://api.example.test", "Path", "yyyyMMdd", resource: "/v1/responses/"), new DateOnly(2026, 9, 18));
        var query = builder.Build(Endpoint("https://api.example.test", "QueryString", "dd-MM-yyyy", resource: "/responses"), new DateOnly(2026, 9, 18));
        Assert.Multiple(() =>
        {
    ...
    public async Task Xml_client_sends_exact_raw_body_and_query_date()
    {
        ...
        var result = await client.SendAsync(Request(raw, Endpoint("https://api.example.test", "QueryString", "yyyy-MM-dd", resource: "/responses")));
    ...
    public async Task Retryable_statuses_create_new_requests_and_acquire_each_permit()
    {
        ...
        var pipeline = new ApiRetryPipeline(settings, limiter, (_, _) => Task.CompletedTask);
        var client = CreateClient(executor, limiter, Endpoint("https://api.example.test", "Path", "yyyy-MM-dd"), pipeline);
        var result = await client.SendAsync(Request("<Request><SchemeCode>S-1</SchemeCode><PolicyReference>Q-1</PolicyReference></Request>", Endpoint("https://api.example.test", "Path", "yyyy-MM-dd")));
        Assert.Multiple(() =>
        {
            Assert.That(result.Successful, Is.True);
    ...
    public async Task Non_retryable_status_and_cancellation_do_not_retry()
    {
        ...
        cancellation.Cancel();
        Assert.That(async () => await client.SendAsync(Request("<Request><SchemeCode>S-1</SchemeCode><PolicyReference>Q-1</PolicyReference></Request>", Endpoint("https://api.example.test", "Path", "yyyy-MM-dd")), cancellation.Token), Throws.InstanceOf<OperationCanceledException>());
    }
    ...
    public async Task Retry_after_and_transport_failures_are_supported_without_network_access()
    {
        ...
        var pipeline = new ApiRetryPipeline(new ResilienceSettings { ApiRetryAttempts = 1, ApiRetryInitialDelaySeconds = 0, ApiRetryMaximumDelaySeconds = 10 }, limiter, (delay, _) => { delays.Add(delay); return Task.CompletedTask; });
        var result = await pipeline.ExecuteAsync("retry-after", (_, _) =>
        {
            attempts++;
            return Task.FromResult(attempts == 1 ? ApiCallResult.Failure(429, "rate limited", attempts, TimeSpan.Zero, TimeSpan.FromSeconds(3)) : ApiCallResult.Success(200, "ok", attempts, TimeSpan.Zero));
        });
        Assert.Multiple(() =>
        {
            Assert.That(result.Successful, Is.True);
    ...
    public async Task Transport_exception_is_retried_and_each_attempt_gets_a_permit()
    {
        var attempts = 0;
        var limiter = new FakeLimiter();
        var pipeline = new ApiRetryPipeline(new ResilienceSettings { ApiRetryAttempts = 1, ApiRetryInitialDelaySeconds = 0, ApiRetryMaximumDelaySeconds = 0 }, limiter, (_, _) => Task.CompletedTask);
        var result = await pipeline.ExecuteAsync("transport", (_, _) =>
        {
            attempts++;
            if (attempts == 1) throw new ApiTransportException("transport");
            return Task.FromResult(ApiCallResult.Success(200, "ok", attempts, TimeSpan.Zero));
        });
        Assert.Multiple(() =>
        {
            Assert.That(result.Successful, Is.True);
    ...
    public void Disabled_invalid_and_incomplete_endpoints_are_rejected_before_network_access()
    {
        ...
        Assert.Multiple(() =>
        {
            Assert.That(() => factory.GetClient("disabled", Endpoint("https://api.example.test", "Path", "yyyy-MM-dd", enabled: false)), Throws.TypeOf<ApiConfigurationException>());
            Assert.That(() => factory.GetClient("invalid", Endpoint("https://endpoint.invalid", "Path", "yyyy-MM-dd")), Throws.TypeOf<ApiConfigurationException>());
            Assert.That(() => factory.GetClient("missing", Endpoint("https://api.example.test", "Path", "yyyy-MM-dd", resource: "")), Throws.TypeOf<ApiConfigurationException>());
            Assert.That(() => factory.GetClient("credentials", Endpoint("https://api.example.test", "Path", "yyyy-MM-dd", username: "", password: "")), Throws.TypeOf<ApiConfigurationException>());
            Assert.That(() => new ApiResourceBuilder().Build(Endpoint("https://api.example.test", "Header", "yyyy-MM-dd"), new DateOnly(2026, 1, 1)), Throws.TypeOf<ApiConfigurationException>());
        });
    }

    private static XmlApiClient CreateClient(FakeExecutor executor, FakeLimiter limiter, EndpointSettings endpoint, IApiRetryPipeline? pipeline = null) => new(new RestClientFactory(), new ApiResourceBuilder(), pipeline ?? new ApiRetryPipeline(new ResilienceSettings { ApiRetryAttempts = 0 }, limiter, (_, _) => Task.CompletedTask), executor);

    private static XmlApiRequest Request(string raw, EndpointSettings endpoint) => new("SCN-1", "S-1", raw, "BUILD-1", endpoint, new RouteSettings { EndpointName = "EndpointA", Enabled = true }, new DateOnly(2026, 9, 18));
    ...
    private sealed class FakeLimiter : IApiRateLimiter
    {
